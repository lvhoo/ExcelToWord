...
public class ZlxxToExcelForWuyu {
	...
	public static void main(String[] args) throws Exception {
		...
		String path = "E:\\wuyu\\专利信息2011.xls";
		...
		for (int filePathIndex = 0; filePathIndex < filePathList.size(); filePathIndex++) {
			...
			for (int i = 6; i <= rowNum - 1; i++) {
				if(rs.getCell(12, i).getContents().trim().equals("2011") || rs.getCell(12, i).getContents().trim().equals("2011年") || (rs.getCell(12, i).getContents().length()>=4 && rs.getCell(12, i).getContents().substring(0,3).equals("2011"))) {
