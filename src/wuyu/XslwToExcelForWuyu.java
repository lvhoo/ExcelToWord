...
public class XslwToExcelForWuyu {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int filePathIndex=0; filePathIndex<filePathList.size(); filePathIndex++) {
			...
			for (int i = 6; i <= rowNum-1; i++) {
				if(rs.getCell(19, i).getContents().trim().equals("2011") || rs.getCell(19, i).getContents().trim().equals("2011年") || (rs.getCell(19, i).getContents().length()>=4 && rs.getCell(19, i).getContents().substring(0,3).equals("2011"))) {
